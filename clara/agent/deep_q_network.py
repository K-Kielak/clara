...
def _generate_random_weights(size, trainable, name):
    return tf.Variable(tf.truncated_normal(size, stddev=0.001, mean=0.002), trainable=trainable, name=name)
...
def _generate_random_biases(size, trainable, name):
    return tf.Variable(tf.truncated_normal(size, stddev=0.001), trainable=trainable, name=name)
