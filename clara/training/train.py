...
MEMORY_SIZE = 250000  # How many experiences to keep in the memory; 250000 ~= 4GB

PRE_TRAIN_STEPS = 100000  # How many steps of random actions before training begins
TRAINING_BATCH_SIZE = 32  # How many experiences to use for each training step
TRAINING_FREQUENCY = 4  # How many actions before performing one training step
NUM_STEPS = 6500000  # How many steps to perform for training session
...
def main():
    ...
    with tf.Session() as sess:
        ...
        print('pre train steps finished')
        # proper training
        for i in range(NUM_STEPS):
            initial_state = environment.get_curr_state_vector()
            dqn_output = dqn.get_online_network_output(initial_state)
